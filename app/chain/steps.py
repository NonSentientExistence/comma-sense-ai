...
class LLMRunner(Runnable[PromptBuilderOutput, LLMRunnerOutput]):
    ...
    def model_post_init(self, __context) -> None:
        ...
        object.__setattr__(self, "pipe", pipeline("text-generation", model=self.model_name))
...
class ResponseParser(Runnable[LLMRunnerOutput, ResponseParserOutput]):
    ...
    def invoke(self, data: LLMRunnerOutput) -> ResponseParserOutput:
        # Strip raw response from LLM
        answer = data.raw_response.strip()

        # Handle case where the LLM returns an empty response or just whitespace
        if not answer:
            answer = f"No answer was generated by {self.model_name}"
